...
public class CareReportController {
  ...
  public SuccessResponse<CareReport> createCareReport(
      @PathVariable Long careGiverID, @RequestBody CareReportRequest careReportRequest) {
    CareReport currentThreeReports =
        careReportService.createCareReport(
            careGiverID,
            careReportRequest.getSpecialNote(),
            careReportRequest.getDescription().getCareSchedulesDescription());
