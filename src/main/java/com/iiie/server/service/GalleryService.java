...
public class GalleryService {
  ...
  public GalleryService(
      GalleryRepository galleryRepository, PatientRepository patientRepository, S3Client s3Client) {
  ...
  public List<GalleryDTO.GetGalleryResponse> getRecentGalleries(Long patientId) {
    ...
    // 최근 이미지 3개 조회
    List<Image> recentImages =
        galleryRepository.findTop3ImagesByPatientIdOrderByIdDesc(patient.getId());
    ...
    for (Image image : recentImages) {
      ...
      responses.add(
          GalleryDTO.GetGalleryResponse.builder()
              .galleryId(gallery.getGallery_id())
              .createdBy(gallery.getCreatedBy())
              .createdAt(gallery.getCreatedAt())
              .title(gallery.getTitle())
  ...
  public void updateGallery(GalleryDTO.UpdateGalleryRequest updateGalleryRequest) {
    ...
    // 이미지 추가
    if (updateGalleryRequest.getAddImages() != null
        && !updateGalleryRequest.getAddImages().isEmpty()) {
      List<Image> imageEntities = new ArrayList<>();
    ...
    // 이미지 삭제
    if (updateGalleryRequest.getDeleteImageIds() != null
        && !updateGalleryRequest.getDeleteImageIds().isEmpty()) {
